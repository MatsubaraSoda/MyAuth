      ...
      <div class="space-y-2">
        <Collapsible v-model:open="isNotebooksOpen" class="w-full">
          <CollapsibleTrigger as-child>
            <button
              class="flex w-full cursor-pointer items-center justify-between rounded-md px-3 py-2 text-base font-medium hover:bg-accent transition-colors"
            >
              {{ t('layout.title_notebooks') }}
              <ChevronDown
          ...
          <CollapsibleContent class="space-y-1 px-4 pt-1">
            <RouterLink
              to="#"
              class="block cursor-pointer rounded-md px-3 py-2 text-sm hover:bg-accent"
              @click="$emit('close')"
              >{{ t('layout.label_subject_math') }}</RouterLink
            >
            <RouterLink
              to="#"
              class="block cursor-pointer rounded-md px-3 py-2 text-sm hover:bg-accent"
              @click="$emit('close')"
              >{{ t('layout.label_subject_physics') }}</RouterLink
            >
            <RouterLink
              to="#"
              class="block cursor-pointer rounded-md px-3 py-2 text-sm hover:bg-accent"
              @click="$emit('close')"
              >{{ t('layout.label_subject_chemistry') }}</RouterLink
            >
          </CollapsibleContent>
        </Collapsible>
      </div>
    ...
    <div class="mt-auto grid gap-4 border-t pt-6">
        ...
        <DropdownMenuTrigger as-child>
          ...
          >
            <div class="flex items-center gap-2">
              ...
              {{ localeLabel(appStore.language) }}
            </div>
            <ChevronDown class="h-4 w-4 opacity-50" />
          </button>
        </DropdownMenuTrigger>
        <DropdownMenuContent side="top" align="end" class="w-(--reka-dropdown-menu-trigger-width)">
          <DropdownMenuItem
          ...
          >
            <span class="flex w-10 shrink-0 items-center justify-center">
              <span
                class="inline-flex max-w-16 justify-center px-1 font-mono text-xs font-semibold leading-snug tracking-wide whitespace-nowrap"
                >{{ loc.code }}</span
              >
            </span>
      ...
      <div class="grid grid-cols-3 gap-2">
        <button
          ...
          :class="{
            ...
            'bg-background text-muted-foreground hover:bg-accent/80 hover:text-accent-foreground':
              appStore.mode !== m,
          }"
      ...
      <div v-else class="grid gap-2">
        <RouterLink
          ...
          @click="$emit('close')"
        >
          <LogIn class="h-4 w-4" /> {{ t('layout.btn_sign_in') }}
        </RouterLink>
        <RouterLink
          to="/auth/sign-up"
          class="inline-flex cursor-pointer items-center justify-center gap-2 rounded-md bg-primary text-primary-foreground px-3 py-2 text-sm hover:bg-primary/90 transition-colors"
          @click="$emit('close')"
        >
          <UserRoundPlus class="h-4 w-4" /> {{ t('layout.btn_sign_up') }}
        </RouterLink>
      </div>
    </div>
  </div>
